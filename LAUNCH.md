...
```
Listen at: http://localhost:9090/stream
```
...
3. Прямая ссылка на поток (для плееров по URL): **http://localhost:9090/stream**
4. Статус (очередь, название): **http://localhost:9090/status**
...
- В браузере по адресу http://localhost:9090 воспроизводится звук (джингл, новости или музыка).
...
3. Выполните:
   ...
   docker-compose up -d
   ```
4. Откройте в браузере: **http://localhost:9090**
...
| Нет звука в браузере | Проверьте, что радио запущено и открыт http://localhost:9090 (не 8000). |
| Нет новостей / только filler | Укажите в `.env` корректный `GROQ_API_KEY` (шаг 5). |
| Порт 9090 занят | Задайте в `.env` переменную `STREAM_PORT` с другим портом, например `8888`. |
